...
If you choose to generate a personal access token with an expiration date, you'll need to generate a new personal access token whenver your old token expires. When you have a new personal access token, you can set that token in RStudio using the library `gitcreds`. When you run the function `gitcreds_set()`, you will be given the option to replace your GitHub security credentials. Select this option (option 2) then paste in the new personal access token. All done!
